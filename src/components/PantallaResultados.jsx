        ...
        if (esFormula) {
            try {
                if (esBloqueExplicativo) {
                    return (
                        <div key={i} className="my-2 p-4 bg-white rounded-2xl border border-slate-100 shadow-sm flex justify-center items-center overflow-x-auto">
...
function PantallaResultados({respuestas, alReiniciar}) {
        ...
        <div className="min-h-screen w-full bg-[#fdfbf7] flex flex-col items-center p-6 md:p-10 font-sans">
            ...
            <div className="w-full max-w-5xl space-y-6">
                {respuestas.map((res, i) => {
                    ...
                    return (
                        <motion.div key={i} className="ficha-viktoria !p-0 !rounded-[1.5rem] overflow-hidden border-none shadow-md mb-4 bg-white">
                            <div className="grid grid-cols-1 md:grid-cols-12">
                                ...
                                <div className="md:col-span-7 p-8 border-b md:border-b-0 md:border-r border-slate-50">
                                    <div className="flex items-center justify-between mb-4">
                                        ...
                                        <span className="text-[8px] font-black text-slate-200 uppercase tracking-tighter flex items-center gap-1">
                                    ...
                                    {res.pregunta.formulaPrincipal && (
                                        <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 flex justify-center mb-8 shadow-inner overflow-x-auto">
    ...
    {res.pregunta.opciones.map((opt, idx) => {
        ...
        return (
            <div key={idx} className={`p-3 rounded-xl border text-xs flex items-center gap-3 ${estiloOpcion}`}>
                <span className="opacity-60 font-black shrink-0">{String.fromCharCode(65 + idx)}:</span>
                ...
                {esLaCorrecta && <CheckCircle2 size={14} className="ml-auto text-emerald-500"/>}
                {esSuEleccion && !esCorrecta && <XCircle size={14} className="ml-auto text-rose-500"/>}
            </div>
        );
    })}
</div>
                                ...
                                <div className="md:col-span-5 p-8 bg-slate-50/50 flex flex-col">
                                    ...
                                    <div className="space-y-4 mt-auto pt-6 border-t border-slate-200">
                                        <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-4">Pasos sugeridos:</p>
            ...
            <div className="w-full max-w-5xl flex flex-col md:flex-row gap-4 mt-12 mb-20 no-print">
                <button onClick={alReiniciar} className="btn-viktoria-primary flex-1 !bg-slate-900 !shadow-none hover:!bg-slate-800">
                    <RotateCcw size={20} className="inline mr-2"/> NUEVO ENTRENAMIENTO
                </button>
                <button onClick={() => window.print()} className="btn-viktoria-ghost flex-1 border-slate-200 text-slate-600">
